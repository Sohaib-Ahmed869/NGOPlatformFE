// Presentational primitives shared by the admin Subscriptions screens (list +
// detail), so a plan reads identically wherever it shows up and mirrors the
// donor "My Subscriptions" look. Pure helpers live in ./subscriptionUtils.
import { Repeat } from "lucide-react";
import { cn } from "../../utils/cn";
import { initials, statusStyle, statusLabel, frequencyStyle, frequencyLabel } from "./subscriptionUtils";
...
export function FrequencyChip({ frequency }) {
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide",
        frequencyStyle(frequency),
      )}
    >
      <Repeat className="h-3.5 w-3.5" /> {frequencyLabel(frequency)}
    </span>
  );
}
