...
export default function SupportSessionBanner() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-[60] flex flex-wrap items-center justify-center gap-x-3 gap-y-1 bg-red-600 px-4 py-2 text-center text-sm font-medium text-white shadow-[0_-4px_18px_-8px_rgba(0,0,0,0.5)]">
      <span className="inline-flex items-center gap-1.5">
        <ShieldAlert className="h-4 w-4 shrink-0" />
        Acting as platform support · {user.impersonatedBy} · {user.slug || ""} — every action is audited
      </span>
      <button
        type="button"
        onClick={endSession}
